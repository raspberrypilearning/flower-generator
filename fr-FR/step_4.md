...
Pour le moment toutes les fleurs que tu dessines sont exactement les mêmes. Ensuite tu vas ajouter quelques entrées au bloc `dessiner une fleur`{:class="block3myblocks"} pour que tu puisses dessiner des fleurs avec des couleurs, tailles et nombres de pétales différents.
...
Pour l'instant toutes les fleurs sont de la même couleur. Pour donner à chaque fleur une couleur différente, tu dois ajouter un bloc **entrée** au bloc `dessiner une fleur`{:class="block3myblocks"}.

--- task ---

Fais un clic droit sur le bloc de définition `dessiner une fleur`{:class="block3myblocks"} et choisis **modifier**:
...
Cette entrée apparaît dans la définition `dessiner une fleur`{:class="block3myblocks"}, et tu peux la glisser là où tu veux l'utiliser.
...
```blocks3
define dessiner une fleur (couleur)
set [color v] effect to (couleur :: custom-arg)
repeat (6)
  stamp
  turn cw (60) degrees
end
...
erase all
go to x: (75) y: (75)
dessiner une fleur (180) :: custom
go to x: (-75) y: (-75)
dessiner une fleur (150) :: custom
```
...
```blocks3
dessiner une fleur (180) (150) :: custom
```
...
Modifie le bloc `dessiner une fleur`{:class="block3myblocks"} pour ajouter une nouvelle entrée appelée «taille».

Tu dois ajouter le bloc suivant avec une entrée «taille» au bloc de définition `dessiner une fleur`{:class="block3myblocks"}:
...
```blocks3
define dessiner une fleur (couleur) (taille :: custom-arg)
set [color v] effect to (couleur :: custom-arg)
set size to (taille :: custom-arg) %
repeat (6) 
  stamp
  turn cw (60) degrees
end
...
dessiner une fleur (180) (150) :: custom
go to x: (-75) y: (-75)
dessiner une fleur (150) (50) :: custom
```
...
```blocks3
define dessiner une fleur (couleur) (taille :: custom-arg) (petals)
```
...
```blocks3
repeat (pétales :: custom-arg) 
end

turn cw ((360) / (pétales :: custom-arg)) degrees
```

Mets à jour tes blocs `dessiner une fleur`{:class="block3myblocks"} pour utiliser l'entrée «pétales».

```blocks3
dessiner une fleur (150) (50) (8) :: custom
```
...
```blocks3
define dessiner une fleur (couleur) (taille :: custom-arg) (pétales)
set [color v] effect to (couleur :: custom-arg)
set size to (taille :: custom-arg) %
repeat (pétales :: custom-arg) 
  stamp
...
erase all
go to x: (75) y: (75)
dessiner une fleur (180) (150) (3) :: custom
go to x: (-75) y: (-75)
dessiner une fleur (150) (50) (8) :: custom
```
